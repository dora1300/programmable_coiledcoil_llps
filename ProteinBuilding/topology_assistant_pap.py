...
parallel and antiparallel coil proteins.
...
Also be mindful that there is *NO* difference in .itp writer for specific vs non-specific regimes, because specificity
is handled at the .top [nonbond_params] level.
...
def atom_helper(aStart, aStop, papSwitch="", segment_type="linker", heptad_start=0, oligo=None, oligoID=0):
    ...
    :param papSwitch: this handles whether a coil multimerizes via parallel or antiparallel orientation,
        There are only two acceptable values = "p" or "ap", must be lowercase.
    ...
    if segment_type == "linker":    # control for making linker information
        ...
        for posc in range(aStart, aStop+1):        # uses 1-indexing
            if posc in a_posits:
                atom_string += f" {posc}   {Alabel}   {posc}    {Aname}     CA    {posc}    0.000    {MASS}\n"
            elif posc in d_posits:
                atom_string += f" {posc}   {Dlabel}   {posc}    {Dname}     CA    {posc}    0.000    {MASS}\n"
